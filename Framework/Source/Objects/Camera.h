...
namespace fw
{
    ...
    public:
        ...
        void setLookAtTarget(GameObject* pTarget) { m_pTarget = pTarget; }

        void onWindowResizeEvent(Event* pEvent);
        void onInputEvent(Event* pEvent);

    protected:
        vec3 m_projectionScale = { 0.1f, 0.1f, 0.1f };
        float32 m_aspectRatio = 1.0f;
        vec3 m_lookAtPosition = vec3(0, 0, 0);

        float m_distance = 10.0f;
        vec3 m_controls = vec3(0,0,0);
        GameObject* m_pTarget = nullptr;

        EventListener m_eventListener;
        EventListener m_inputEventListener;
    };
